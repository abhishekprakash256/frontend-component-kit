...
}, ob = (e) => e || (window == null ? void 0 : window.history), Sb = ({ router: e }) => {
  const [t, n] = Ze("light"), [r, i] = Ze(!1), [a, o] = Ze(""), l = ob(e);
  je(() => {
    const c = localStorage.getItem("theme") || "light";
    n(c), i(!0);
  }, []), je(() => {
    r && (document.documentElement.setAttribute("data-theme", t), localStorage.setItem("theme", t));
  }, [t, r]);
  const s = () => {
    n((c) => c === "light" ? "dark" : "light");
  }, u = (c) => {
    c.preventDefault(), a.trim() && (l && typeof l.push == "function" ? l.push(`/search/${encodeURIComponent(a)}`) : window.location.href = `/search/${encodeURIComponent(a)}`);
  };
